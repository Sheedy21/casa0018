...
With all the images labeled, they were divided for the train and test data. For this step was used two scripts from Dat Tran’s raccoon detector. The first file is "xml_to_csv.py" which exports two csv files, one for the train data and the other for the test data. The other script used was "generate_tfrecord.py" which generates the TFRecords.
...
For the deployment of the models as Edge AI, the Tensorflow Lite app lets use the custom object detector in different operating systems considering an input image size of (300x300x3). I couldn't replicate it on my device because it didn't meet the requirements.

## Results and Observations

According to the results of the three experiments, we can see that the size of the images considered to train the model can help improve the face detection model. Even though the final loss of the first model is lower than the second, this late presented better results when it was tested in a video frame. One observation made to the first two models is the data size. In these experiments, it was just considered 55 images per each one because the process to label all the images was done manually. Another problem identified with these models is that they cannot distinguish between the eyes of a person and an animal. Concerning the last experiment, the model was able to recognize various faces with different probabilities of response, but the main idea of this project is to identify partial faces too. For this reason, if we combine the last two models, we can get a more accurate model, but for this it will be necessary to train the object detector with partial faces, not with too small parts like the eye, nose, or mouth but with larger parts of the face like in the graphs below.

![alt text](https://github.com/Sheedy21/casa0018/raw/main/Assessment/Reports/images/face_div.jpg)
