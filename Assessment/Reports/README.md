...
According to the results of the three experiments, we can see that the size of the images considered to train the model can help improve face detection. Another factor that can be considered for future works would be the number of images. In these experiments, it was just considered 55 images per each one because this process was done manually. One problem with this model would be that this can't distinguish between the eyes of a person and an animal. Concerning the last experiment, the model was able to recognize various faces with different probabilities of response. Combining the last two models, we can conclude that to get a more accurate model is necessary to train the object detector with partial faces, not with too small parts like the eye, nose, or mouth but with larger parts of the face like in the graphs below.
